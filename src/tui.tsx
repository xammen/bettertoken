...
function loadCfg(api: TuiPluginApi, initial: Config): Config {
...
function record(api: TuiPluginApi, msg: AssistantMessage) {
...
function FooterView(props: { api: TuiPluginApi; cfg: () => Config; tick: () => number }) {
...
function InlineView(props: { api: TuiPluginApi; cfg: () => Config; sid: string; tick: () => number }) {
  const data = createMemo(() => {
    props.tick()
    const entries = readDisk(props.api).entries
    const c = props.cfg()
